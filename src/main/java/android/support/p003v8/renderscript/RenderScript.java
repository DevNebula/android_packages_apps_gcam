...
public class RenderScript {
    ...
    public static RenderScript create(Context context, int i) {
        return RenderScript.create(context, i, ContextType.NORMAL, 0);
    }

    public static RenderScript create(Context context, int i, ContextType contextType) {
        return RenderScript.create(context, i, contextType, 0);
    }
    ...
    public static RenderScript create(Context context, int i, ContextType contextType, int i2) {
        ...
        synchronized (mProcessContextList) {
            ...
            renderScript = RenderScript.internalCreate(context, i, contextType, i2);
    ...
    public static RenderScript create(Context context, ContextType contextType, int i) {
        return RenderScript.create(context, context.getApplicationInfo().targetSdkVersion, contextType, i);
    }

    public static RenderScript createMultiContext(Context context, ContextType contextType, int i, int i2) {
        return RenderScript.internalCreate(context, i2, contextType, i);
    }
    ...
    private static RenderScript internalCreate(Context context, int i, ContextType contextType, int i2) {
        ...
        useNative = RenderScript.setupNative(sSdkVersion, context);
        synchronized (lock) {
            if (!sInitialized) {
                try {
            ...
            if (sNative == 1) {
                try {
                    ...
                    if (applicationInfo.metaData != null) {
                        if (applicationInfo.metaData.getBoolean("com.android.support.v8.renderscript.EnableAsyncTeardown") && longValue == 0) {
                            sNative = 0;
                        }
