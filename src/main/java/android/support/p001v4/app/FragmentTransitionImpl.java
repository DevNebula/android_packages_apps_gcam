...
public abstract class FragmentTransitionImpl {
    ...
    public static void bfsAddViewChildren(List list, View view) {
        int size = list.size();
        if (!FragmentTransitionImpl.containedBeforeIndex(list, view, size)) {
            list.add(view);
            for (int i = size; i < list.size(); i++) {
                ...
                if (view2 instanceof ViewGroup) {
                    ...
                    for (int i2 = 0; i2 < childCount; i2++) {
                        ...
                        if (!FragmentTransitionImpl.containedBeforeIndex(list, childAt, size)) {
                            list.add(childAt);
                        }
                    }
                }
    ...
    ArrayList prepareSetNameOverridesReordered(ArrayList arrayList) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            arrayList2.add(C0315jm.m4641p(view));
            C0315jm.m4617a(view, null);
        }
        return arrayList2;
    }
    ...
    void scheduleNameReset(ViewGroup viewGroup, final ArrayList arrayList, final Map map) {
        OneShotPreDrawListener.add(viewGroup, new Runnable() {
            public void run() {
                ...
                for (int i = 0; i < size; i++) {
                    View view = (View) arrayList.get(i);
                    C0315jm.m4617a(view, (String) map.get(C0315jm.m4641p(view)));
                }
            }
        });
    }
        ...
        OneShotPreDrawListener.add(view, new Runnable() {
            public void run() {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    String p = C0315jm.m4641p(view);
                    if (p != null) {
                        C0315jm.m4617a(view, FragmentTransitionImpl.findKeyForValue(map, p));
                    }
                }
            }
        });
    ...
    void setNameOverridesReordered(View view, ArrayList arrayList, ArrayList arrayList2, ArrayList arrayList3, Map map) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            String p = C0315jm.m4641p(view2);
            arrayList4.add(p);
            if (p != null) {
                C0315jm.m4617a(view2, null);
                String str = (String) map.get(p);
                for (int i2 = 0; i2 < size; i2++) {
                    if (str.equals(arrayList3.get(i2))) {
                        C0315jm.m4617a((View) arrayList2.get(i2), p);
                        break;
                    }
                }
            }
        ...
        OneShotPreDrawListener.add(view, new Runnable() {
            public void run() {
                ...
                while (true) {
                    int i2 = i;
                    if (i2 < size) {
                        C0315jm.m4617a((View) arrayList5.get(i2), (String) arrayList6.get(i2));
                        C0315jm.m4617a((View) arrayList7.get(i2), (String) arrayList4.get(i2));
                        i = i2 + 1;
                    } else {
                        return;
                    }
