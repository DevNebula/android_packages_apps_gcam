...
class NotificationCompatJellybean {
    ...
    private static RemoteInput[] fromBundleArray(Bundle[] bundleArr) {
        ...
        for (int i = 0; i < bundleArr.length; i++) {
            remoteInputArr[i] = NotificationCompatJellybean.fromBundle(bundleArr[i]);
        }
    ...
    public static Action getAction(Notification notification, int i) {
        synchronized (sActionsLock) {
            try {
                Object[] actionObjectsLocked = NotificationCompatJellybean.getActionObjectsLocked(notification);
                if (actionObjectsLocked != null) {
                    ...
                    Action readAction = NotificationCompatJellybean.readAction(sActionIconField.getInt(obj), (CharSequence) sActionTitleField.get(obj), (PendingIntent) sActionIntentField.get(obj), bundle);
                    return readAction;
                }
            } catch (Throwable e) {
                Log.e(TAG, "Unable to access notification actions", e);
    ...
    static Action getActionFromBundle(Bundle bundle) {
        ...
        return new Action(bundle.getInt("icon"), bundle.getCharSequence(KEY_TITLE), (PendingIntent) bundle.getParcelable(KEY_ACTION_INTENT), bundle.getBundle("extras"), NotificationCompatJellybean.fromBundleArray(NotificationCompatJellybean.getBundleArrayFromBundle(bundle, KEY_REMOTE_INPUTS)), NotificationCompatJellybean.fromBundleArray(NotificationCompatJellybean.getBundleArrayFromBundle(bundle, KEY_DATA_ONLY_REMOTE_INPUTS)), z, bundle.getInt(KEY_SEMANTIC_ACTION), bundle.getBoolean(KEY_SHOWS_USER_INTERFACE));
    }
    ...
    static Bundle getBundleForAction(Action action) {
        ...
        bundle2.putParcelableArray(KEY_REMOTE_INPUTS, NotificationCompatJellybean.toBundleArray(action.getRemoteInputs()));
    ...
    public static Action readAction(int i, CharSequence charSequence, PendingIntent pendingIntent, Bundle bundle) {
        ...
        if (bundle != null) {
            fromBundleArray = NotificationCompatJellybean.fromBundleArray(NotificationCompatJellybean.getBundleArrayFromBundle(bundle, NotificationCompatExtras.EXTRA_REMOTE_INPUTS));
            remoteInputArr = NotificationCompatJellybean.fromBundleArray(NotificationCompatJellybean.getBundleArrayFromBundle(bundle, EXTRA_DATA_ONLY_REMOTE_INPUTS));
    ...
    private static Bundle[] toBundleArray(RemoteInput[] remoteInputArr) {
        ...
        for (int i = 0; i < remoteInputArr.length; i++) {
            bundleArr[i] = NotificationCompatJellybean.toBundle(remoteInputArr[i]);
        }
        return bundleArr;
    }
    ...
    public static Bundle writeActionAndGetExtras(Builder builder, Action action) {
        ...
        if (action.getRemoteInputs() != null) {
            bundle.putParcelableArray(NotificationCompatExtras.EXTRA_REMOTE_INPUTS, NotificationCompatJellybean.toBundleArray(action.getRemoteInputs()));
        }
        if (action.getDataOnlyRemoteInputs() != null) {
            bundle.putParcelableArray(EXTRA_DATA_ONLY_REMOTE_INPUTS, NotificationCompatJellybean.toBundleArray(action.getDataOnlyRemoteInputs()));
        }
        bundle.putBoolean(EXTRA_ALLOW_GENERATED_REPLIES, action.getAllowGeneratedReplies());
        return bundle;
    }
