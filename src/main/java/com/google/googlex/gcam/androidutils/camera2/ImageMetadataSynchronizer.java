...
public class ImageMetadataSynchronizer {
    ...
    class C01682 extends CaptureCallback {
        C01682() {
        }
        ...
        public void close() {
            ...
            while (true) {
                ...
                if (i2 < this.mImages.size()) {
                    Image image = (Image) this.mImages.get(i2);
                    if (image != null) {
                        image.close();
                        this.mSynchronizer.notifyImageClosed(i2);
                    }
                    i = i2 + 1;
                } else {
                    this.mImages.clear();
    ...
    private void createCaptureCallback() {
        this.mCaptureCallback = new C01682();
    }
    ...
    private boolean sweepImageQueues(Output output) {
        ...
        while (allIndexedImagesNotNull(output, captureRequestTag.mTargets)) {
            ...
            if (i4 == 0) {
                ...
                while (i2 < i4) {
                    i3 = i2 + 1;
                    i2 = ((Integer) arrayList3.get(i2)).intValue();
                    if (longValue < ((Image) output.mImages.get(i2)).getTimestamp()) {
                        output.mDroppedImageReaderIndices.add(Integer.valueOf(i2));
                        output.mImages.set(i2, null);
                        output.mImagesWereDropped = true;
                        i2 = i3;
                    } else {
                        ((LinkedList) this.mPendingImageQueues.get(i2)).removeFirst();
                        i2 = i3;
                    }
                }
                return true;
